...
class AntennaArray(object):
    ...
    def bp_sinc(self, width):
        ...
        return [np.sinc((ii-half_antennas)/np.pi) for ii in range(self.N_antennas)]
    ...
    def array_response(self, bp, x):
        return np.dot(bp, np.array(2j*np.pi*np.arange(self.N_antennas)[:, np.newaxis]*np.array(x)[np.newaxis, :]))

Antenna = AntennaArray(16, 0.5)
...
R_sinc = Antenna.array_response(bp_sinc, angle_domain)
R_cheby = Antenna.array_response(bp_cheby, angle_domain)
